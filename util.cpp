...
string UTIL_EscapedToString( const string &strEscape )
{
	...
	for( unsigned long i = 0; i < strEscape.size( ); )
	{
		if( strEscape[i] == '%' )
		{
			...
			{
				printf( "error decoding escaped string - possible truncation, halting decode\n" );
...
void UTIL_MoveFile( const char *szFile, const char *szDest )
{
	if( UTIL_CheckFile( szDest ) )
		printf( "error archiving \"%s\" - destination file already exists\n", szDest );
	else
...
CAtomList *UTIL_DecodeHTTPPost( string strPost )
{
	...
	while( 1 )
	{
		...
		{
			printf( "error decoding HTTP POST request - unexpected end of request\n" );
		...
		{
			printf( "error decoding HTTP POST request - couldn't find Content-Disposition\n" );
		...
		{
			printf( "error decoding HTTP POST request - malformed Content-Disposition\n" );
		...
		{
			printf( "error decoding HTTP POST request - malformed segment\n" );
